...
def extract_features_by_province(df):
    """Extract optimized features per province for clustering (based on correlation analysis)"""
    print("\nExtracting optimized features by province...")
    ...
    for province in tqdm(df['province'].unique()):
        ...
        # Core non-redundant features based on correlation analysis
        total_quakes = len(province_data)
        major_quakes = len(province_data[province_data['magnitude'] >= 4.0])  # m4_plus
        avg_magnitude = province_data['magnitude'].mean()
        
        # Magnitude distribution (use m3_plus instead of m2_plus due to high correlation)
        m3_plus = len(province_data[province_data['magnitude'] >= 3.0])
        
        # High-magnitude percentage
        high_mag_percentage = (m3_plus / max(total_quakes, 1)) * 100
        
        # Low-magnitude ratio (distinct from high magnitude)
...
def cluster_base_risk(provinces_features, n_clusters=2):
    """Cluster provinces based on optimized non-redundant features"""
    print("\nClustering provinces based on optimized features...")
    
    # Select optimized features (removed redundant ones based on correlation analysis)
    # Removed: m2_plus_count, m4_plus_count, earthquake_rate (high correlation with total_quakes)
    # Removed: median_magnitude (0.985 correlation with avg_magnitude)
    # Removed: temporal_variation (weak predictive power)
    features = [
        'total_quakes',           # Core metric
        'major_quakes',           # m4_plus (1.0 corr with major_quakes, so use major_quakes directly)
        'avg_magnitude',          # Keep median would be redundant
        'high_mag_percentage',    # Derived from m3_plus, different from total
        'low_mag_ratio',          # Inverse pattern from high_mag_ratio (-0.837 with mid)
        'activity_rate',          # Normalized by time
        'recent_activity',        # Recent trend
        'shallow_ratio',          # Depth characteristic
    ]
...
def main():
    print("="*70)
    print("PROVINCE RISK CLUSTERING V3 (OPTIMIZED - LOW REDUNDANCY)")
    print("="*70)
    print("\nFeatures selected based on correlation analysis:")
    print("- Removed: m2_plus_count, m4_plus_count (high correlation with total_quakes)")
    print("- Removed: median_magnitude (0.985 corr with avg_magnitude)")
    print("- Removed: earthquake_rate (0.808 corr with total_quakes)")
    print("- Kept: total_quakes, major_quakes, avg_magnitude, activity ratios, recent activity")
    print("="*70)
    ...
    # Display results
    print("\n" + "="*70)
    print("RISK-BASED CLUSTERING RESULTS (V3 - OPTIMIZED)")
    print("="*70)
    ...
    # Plot 4: High Magnitude % vs Recent Activity
    ax4 = axes[1, 1]
    for risk_level in ['Low', 'Medium']:
        data = provinces_features[provinces_features['risk_level'] == risk_level]
        ax4.scatter(data['high_mag_percentage'], data['recent_activity'], 
                   s=100, alpha=0.6, label=risk_level, color=risk_colors[risk_level],
                   edgecolor='black', linewidth=0.5)
    
    ax4.set_xlabel('High Magnitude % (M≥3.0)', fontsize=11, fontweight='bold')
    ax4.set_ylabel('Recent Activity (Last 30 days)', fontsize=11, fontweight='bold')
    ax4.set_title('High Magnitude % vs Recent Activity by Risk', fontsize=12, fontweight='bold')
    ax4.legend()
    ...
    # Save the plot
    output_path = os.path.join(os.path.dirname(__file__), 'province_risk_clustering_v3.png')
