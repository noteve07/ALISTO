...
class EarthquakeDataFetcher:
    def __init__(self):
        ...
        # Load province ID to name mapping
        self.province_mapping = self._load_province_mapping()
        logger.info(f"✅ Loaded {len(self.province_mapping)} provinces from local mapping")
    ...
    def fetch_earthquakes_last_30_days(self) -> pd.DataFrame:
        ...
        try:
            ...
            for eq in response.data:
                # Get province name from local mapping
                province_name = self.province_mapping.get(eq['province_id'], 'Unknown')
    ...
    def fetch_all_provinces(self) -> List[Dict]:
        """Get all provinces from local mapping"""
        try:
            provinces = []
            ...
            logger.info(f"✅ Loaded {len(provinces)} provinces from local mapping")
            return provinces
            
        except Exception as e:
            logger.error(f"Error loading provinces from local mapping: {e}")
            raise
