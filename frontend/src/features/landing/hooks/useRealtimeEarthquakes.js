 ...
 * Copied from live-monitoring for landing page
  ...
  useEffect(() => {
    ...
    channel.subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('✅ Subscribed to earthquake updates (Landing Page)')
      } else if (status === 'CHANNEL_ERROR') {
        console.error('❌ Error subscribing to earthquake updates (Landing Page)')
      } else if (status === 'TIMED_OUT') {
        console.warn('⏰ Subscription timed out (Landing Page)')
      } else if (status === 'CLOSED') {
        console.log('🔒 Subscription closed (Landing Page)')
      }
    })
