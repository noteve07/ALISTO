...
const AuthBrandSection = ({ type = "login" }) => {
  ...
  return (
    <div
      ...
      style={{
        animation: isLogin
          ? "slideFromLeft 0.5s ease-out"
          : "slideFromRight 0.5s ease-out",
      }}
    >
      {/* Decorative elements */}
      <div
        className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 transition-all duration-1000"
        style={{
          animation: "float 6s ease-in-out infinite",
        }}
      ></div>
      <div
        className="absolute bottom-0 left-0 w-96 h-96 bg-orange-500/20 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2 transition-all duration-1000"
        style={{
      ...
      <div className="relative z-10 mt-8">
        <div className="bg-white/5 backdrop-blur-sm rounded-lg p-6 border border-white/10">
          ...
          <button
            ...
            className="w-full py-2.5 px-4 bg-white/10 text-white border border-white/30 rounded-lg font-medium hover:bg-white/20 transition-all duration-200"
          >
