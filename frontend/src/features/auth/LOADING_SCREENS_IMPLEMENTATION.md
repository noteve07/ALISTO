...
1. **User clicks "Logout"** in sidebar → `LogoutLoadingScreen` (1.5 seconds) → Landing page
...
**Location**: `frontend/src/features/auth/components/LogoutLoadingScreen.jsx`

**Purpose**: Loading screen shown during logout process

**Features**:

- Gradient background matching brand colors (primary-v2 → primary)
- Floating orbs with different animation speeds
- Triple rotating rings with logout icon
- "Thank you" message with fade effect
...
- `message` (string): The loading message to display
  - Default: "Signing you out..."

**Animations**:

- Orbs: Floating animation at 6s and 8s speeds (float)
- Outer ring: 2s rotation (spin)
- Middle ring: 1.5s reverse rotation (spin)
- Inner ring: 1s rotation (spin)
- Center icon: Pulsing animation (pulse)
- Loading dots: Bouncing with staggered delays (bounce)
- Thank you message: Pulse animation
...
- Added state management for logout loading
- Shows `LogoutLoadingScreen` during logout process
- Includes 1.5-second delay before redirecting to landing page
- Properly clears logout state on errors
...
### Logout Loading Phase

- **Duration**: 1.5 seconds
- **Purpose**: Smooth transition from dashboard to landing page with goodbye message
- **User sees**: Rotating rings, logout icon, and thank you message
...
- [ ] Logout from sidebar shows logout loading → landing page
...
- [ ] Logout loading properly clears on errors
