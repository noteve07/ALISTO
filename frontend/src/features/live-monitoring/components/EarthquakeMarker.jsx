...
const EarthquakeMarker = ({ event, isLatest }) => {
  ...
  useEffect(() => {
    ...
    const getMagnitudeColorForPopup = (magnitude) => {
      ...
      if (magnitude >= 4)
        return "text-[#D2691E] bg-orange-50 border-orange-200";
      if (magnitude >= 3)
    ...
    const popupContent = `
      <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="${getMagnitudeColorForPopup(event.magnitude)
          .replace("text-", "bg-")
          .replace("bg-red-600", "bg-red-100")
          .replace("bg-orange-600", "bg-orange-100")
          .replace("bg-[#D2691E]", "bg-orange-100")
          .replace("bg-yellow-600", "bg-yellow-100")
          .replace("bg-green-600", "bg-green-100")} px-4 py-3">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-sm font-semibold text-gray-800">${
                isLatest ? "Earthquake Alert" : "Earthquake"
              }</h3>
            </div>
            <div class="text-xs text-gray-600">${formatTimeAgo(
              event.timestamp
            )}</div>
          </div>
        </div>
        ...
        <div class="p-4 space-y-3">
          ...
          <div class="flex items-center gap-3">
            <div class="px-2 py-1 rounded border ${getMagnitudeColorForPopup(
              event.magnitude
            )}">
          ...
          <div class="pt-2 border-t border-gray-100">
            <div class="text-xs text-gray-600">${
              event.dateTime || "Unknown time"
            }</div>
          </div>
          
          ${
            isLatest
              ? '<div class="mt-2 px-2 py-1 bg-blue-50 border border-blue-200 rounded"><span class="text-xs font-semibold text-blue-600">🔴 Latest Earthquake</span></div>'
              : ""
          }
        </div>
      </div>
    ...
    if (isLatest) {
      ...
      const createPulseLayer = (delay) => {
        ...
        cancelAnimations.push(
          startPulseAnimation(pulseLayer, radius, color, delay)
        );
      };
