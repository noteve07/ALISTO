...
const MapLegend = () => {
  const magnitudeLegend = [
    ...
    { range: '4.0-4.9', color: 'bg-[#D2691E]', size: 'w-3.5 h-3.5', label: 'Moderate' },
    { range: '3.0-3.9', color: 'bg-yellow-500', size: 'w-3 h-3', label: 'Light' },
    { range: '<3.0', color: 'bg-green-500', size: 'w-2.5 h-2.5', label: 'Minor' }
  ]
        ...
        <div className="space-y-1.5">
          {magnitudeLegend.map((item, index) => (
            <div key={index} className="flex items-center gap-2">
              <div className={`${item.size} ${item.color} rounded-full shadow-sm shrink-0`} />
